...
rsync -avHK --delete --exclude '*data/ldap' --backup --backup-dir=$BACKUPDIR /opt/zimbra/ $DESTLOCAL/zimbra
...
test=$BACKUPDIR"/zimbra-ldap"
mkdir -p "$test"
chown zimbra. "$test"
echo 'Saving ldap config'
su zimbra -c "/opt/zimbra/libexec/zmslapcat -c $BACKUPDIR/zimbra-ldap"
echo 'Saving ldap main'
su zimbra -c "/opt/zimbra/libexec/zmslapcat $BACKUPDIR/zimbra-ldap"
